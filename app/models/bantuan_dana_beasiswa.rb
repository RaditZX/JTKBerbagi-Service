class BantuanDanaBeasiswa < ApplicationRecord
  ...
  self.primary_key = ['bantuan_dana_beasiswa_id', 'mahasiswa_id', 'penggalangan_dana_beasiswa_id']

  belongs_to :penggalangan_dana_beasiswa, class_name: "PenggalanganDanaBeasiswa", 
               foreign_key: :penggalangan_dana_beasiswa_id
  belongs_to :mahasiswa, class_name: "Mahasiswa", 
               foreign_key: :mahasiswa_id, primary_key: :nim
  ...
  scope :rekapitulasi, -> { where("status_pengajuan = ? OR status_pengajuan = ?", Enums::StatusPengajuan::APPROVED, Enums::StatusPengajuan::DONE) }

  has_many :evaluasi_penyaluran_beasiswas, foreign_key: :bantuan_dana_beasiswa_id
end